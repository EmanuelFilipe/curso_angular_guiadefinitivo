...
app.post('/login', (req, res) => {
    ...
    const USER_FOUND = USERS_LIST_BD.find(u => u.username === username && u.password === password)
...
app.put('/update-user', authenticateToken, (req, res) => {
    ...
    if(!name || !email || !username || !password)
        return res.status(400).json({ message: 'All fields (name, email, username, passwrod) are required' })

    const USER_FOUND = USERS_LIST_BD.findIndex(u => u.username === tokenUserName)
...
app.post('/create-user', authenticateToken, (req, res) => {
    ...
    const USER_TOKEN_FOUND = USERS_LIST_BD.findIndex(u => u.username === tokenUserName)
